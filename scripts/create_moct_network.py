"""
Create Regional Network V3 - MOCT Only

This script creates a MATSim network using ONLY the MOCT Standard Node Link data.
This avoids the complexity and inconsistency of merging OSM and MOCT networks.

The MOCT data covers the entire study area including Jeonggwan-eup with consistent
road hierarchy, lane counts, and speed limits.
"""
...
OUTPUT_FILE = "input/regional-network-expanded.xml"

# Study area BBox - Expanded to include:
# 부산, 울산, 양산, 김해, 경주, 포항, 밀양, 창원
STUDY_BBOX_WGS84 = {
    'minx': 128.5,  # West  (창원 포함)
    'miny': 35.0,   # South (부산 남부 포함)
    'maxx': 129.6,  # East  (포항 동부 포함)
    'maxy': 36.1    # North (포항/경주 포함)
}
...
def load_moct_network():
    """Load and filter MOCT network for study area."""
    ...
    # Keep original CRS (5186) or convert to 5179
    # Using 5179 for consistency with existing project
    moct_links = moct_links.to_crs(epsg=5179)
    moct_nodes = moct_nodes.to_crs(epsg=5179)
    
    # Create bounding box in EPSG:5179
    transformer = Transformer.from_crs("EPSG:4326", "EPSG:5179", always_xy=True)
    minx, miny = transformer.transform(STUDY_BBOX_WGS84['minx'], STUDY_BBOX_WGS84['miny'])
    maxx, maxy = transformer.transform(STUDY_BBOX_WGS84['maxx'], STUDY_BBOX_WGS84['maxy'])
    study_bbox = box(minx, miny, maxx, maxy)
    
    print(f"  Study area bbox (5179): {minx:.0f}, {miny:.0f}, {maxx:.0f}, {maxy:.0f}")
...
def write_network(nodes_df, links_df, output_file):
    ...
    root.set('name', 'Jeonggwan Region - MOCT Only')
